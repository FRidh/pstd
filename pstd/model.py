...
class Axis(object):
    ...
    def wavenumbers(self):
        ...
        Wavenumbers.
        """
        return wavenumbers(self.nodes, self.spacing).astype(
            self._axes._model.dtype("float"), copy=False
        )
...
class PointSource(Source):
    ...
    def gaussian_pulse(amplitude, spacing, shape=(15, 15), a=0.3):
        ...
        pulse = amplitude * np.exp(
            -g * sum([(grid - pos) ** 2.0 for grid, pos in zip(grids, position)])
        )
        return pulse
    ...
    def mass(self):
        ...
        if self.quantity == "pressure":
            m = mass(
                self.amplitude,
                self._model.medium.soundspeed_mean,
                self._model.grid.spacing,
                self._model.ndimensions,
            )
            try:
                return float(m)
            except TypeError:
    ...
    def _attributes(self):
        ...
        source_fields = [
            "position",
            "position_with_pml",
            "position_without_pml",
            "position_in_nodes",
            "position_in_nodes_with_pml",
            "position_in_nodes_without_pml",
            "quantity",
            "component",
            "excitation",
            "amplitude",
    ...
    def _field_sine(self):
        ...
        while True:
            ...
            field[(position[0], position[1])] = amplitude * np.sin(
    ...
    def _field_pulse(self):
        ...
        pulse_grid = np.zeros(
            self._model.grid.shape, dtype=self._model.dtype("float")
        )  # Grid
        try:
            pulse_grid[
                position[0] - offset[0] : position[0] - offset[0] + shape[0],
                position[1] - offset[1] : position[1] - offset[1] + shape[1],
            ] = pulse
        except ValueError:
            raise ValueError(
                "Source is too close to the border. Cannot fit Gaussian pulse."
            )
...
class Receiver(Transducer):
    ...
    def recording(self):
        """Get recording."""
        return acoustics.Signal(self._data, fs=self._model.temporal_sample_frequency)
...
class Medium(object):
    ...
    def soundspeed_for_calculation(self):
        ...
        if not isinstance(self.soundspeed, collections.abc.Iterable):
        ...
        # Generally the case when have no PML.
            ...
            len(self.soundspeed) == 1 or self.soundspeed.shape == self._model.grid.shape
        ):
            return self.soundspeed
        else:
            if not self.soundspeed.shape == self._model.grid.shape_without_pml:
                raise ValueError(
                    "Soundspeed has dimensions {} while a single value or shape {} is required.".format(
                        self.soundspeed.shape, self._model.grid.shape_without_pml
                    )
                )
...
class PML(object):
    ...
    def __init__(self, absorption_coefficient=None, depth=0.0):
        ...
        self.absorption_coefficient = (
            absorption_coefficient if absorption_coefficient else (0.0, 0.0)
        )
        """
    ...
    def generate_grid(self):
        ...
        pml["y"][:, :+nodes] = np.tile(
            self.absorption_coefficient_grid(
                np.linspace(-depth, 0, nodes),
                0.0,
                -depth,
                alpha_max=self.absorption_coefficient[1],
            ),
            (shape[0], 1),
        )  # Top
        pml["y"][:, -nodes:] = np.tile(
            self.absorption_coefficient_grid(
                np.linspace(0, +depth, nodes),
                0.0,
                +depth,
                alpha_max=self.absorption_coefficient[1],
            ),
        ...
        pml["x"][:+nodes, :] = np.tile(
            self.absorption_coefficient_grid(
                ...
                0.0,
                -depth,
                alpha_max=self.absorption_coefficient[0],
            ),
            (shape[1], 1),
        ).T  # Left
        pml["x"][-nodes:, :] = np.tile(
    ...
    def absorption_coefficient_grid(x, x_0, depth, alpha_max, m=4.0):
        ...
        # return 50.0
        return (
            alpha_max * ((x - x_0) / (depth * 1.1)) ** m
        )  # 1.01 to prevent division by zero
...
class Grid(object):
    ...
    def spacing(self):
        ...
        if self._spacing:
            ...
            return (
                np.min(self._model.medium.soundspeed) / self.spatial_sample_frequency
            )  # (2.0 * self._model.maximum_frequency)
...
class Model(object, metaclass=abc.ABCMeta):
    ...
    def timestep(self):
        ...
        if self._timestep:
            ...
            return (
                self.cfl
                * self.grid.spacing
                / float(np.max(self.medium.soundspeed))
                / self.ndimensions
            )
    ...
    def _post_update(self, data):
        ...
        for receiver in self.receivers:
            receiver._data.append(
                data["field"][(receiver.quantity, receiver.component)][
                    receiver._position_in_nodes
                ].real
            )
    ...
    def run(self, steps=None, seconds=None):
        ...
        """
        if (steps is None or steps == 0) and (seconds is None or seconds <= 0.0):
            raise ValueError("Amount of steps or seconds needs to be specified.")
        elif steps is not None and seconds is not None:
            raise ValueError(
                "Either amount of steps or seconds needs to be specified, not both."
            )
    ...
    def overview(self):
        ...
        return (
            ...
            + "Sample frequency temporal: {:.1f}\n".format(
                self.temporal_sample_frequency
            )
            + "Sample frequency spatial: {:.1f}\n".format(
                self.grid.spatial_sample_frequency
            )
    ...
    def _source_arrays_generator(self):
        ...
        while True:
            ...
            for source in self.sources:
                sources[(source.quantity, source.component)] += next(
                    source._field_generator
                )
    ...
    def plot_scene(self):
        ...
        ax.scatter(
            x, y, s=100, label="Source", marker="x", color="r"
        )  # , markersize=10)
        ...
        ax.scatter(
            x, y, s=100, label="Receiver", marker="o", color="g"
        )  # s, markersize=10)
        ...
        ax.add_patch(
            plt.Rectangle(
                (0.0, self.axes.y.length), self.axes.x.length, -self.pml.depth
            )
        )
        ...
        ax.add_patch(
            plt.Rectangle(
                (self.axes.x.length, 0.0), -self.pml.depth, self.axes.y.length
            )
        )
    ...
    def plot_field(self, quantity="pressure", component=None):
        """
        Plot field.
        ...
        if (
            model.settings["pml"]["use"]
            and not model.settings["pml"]["include_in_output"]
        ):
...
def line_receiver_array(
    model, name, start, stop, n=None, spacing=None, quantity=None, component=None
):
...
def grid_receiver_array(
    model, name, center, x, y, n=None, spacing=None, quantities=None
):
...
QUANTITIES = {"pressure": "Pa", "velocity": "m/s"}
